...
OLCI field of view and the complete paired SWOT KaRIn scene. The left and right
KaRIn swaths are kept in the same catalogue record rather than emitted as two
independent vignettes. The platform is selected for each run from the command
line. Candidate scenes are constrained in space, time, latitude, combined
surface area, ocean fraction, and along-track length.
...
- **SWOT KaRIn:** two disjoint swaths, 10 to 60 km from nadir on the left and
  right sides, stored together as one scene. A complete scene therefore spans
  approximately 120 km across track, including the 20 km nadir gap.
...
6. Build OLCI and both KaRIn polygons in local azimuthal-equidistant
   projections.
7. Union the left and right intersections into one paired-swath scene, then
   split long intersections into along-track scenes no longer than 50 km.
8. Calculate combined and per-swath area and ocean fraction using Natural
   Earth land polygons.
9. Export a GeoPackage with a scene layer and a companion swath-geometry layer,
   plus a CSV scene catalogue.
...
- a `vignettes` GeoPackage layer with one complete, generally multipolygon,
  geometry per paired-swath SWOT scene;
- a `swaths` GeoPackage layer with the corresponding left and right geometries,
  linked by `vignette_id`;
- a CSV containing the scene attributes without geometry.

Important scene fields include acquisition times, absolute time separation,
cycle/pass/revolution identifiers, along-track limits, combined area, ocean
area, ocean percentage, centre coordinates, and processing parameters.
`left_area_km2`, `right_area_km2`, `left_ocean_percent`,
`right_ocean_percent`, `left_dt_minutes`, and `right_dt_minutes` preserve the
per-swath diagnostics.

Catalogues generated by versions that emitted one row per side must be
regenerated. The clear-sky screener intentionally rejects those legacy files
because they do not contain the required `swaths` layer.
...
### Clear-sky screening of paired scenes

Cloud coverage is evaluated independently on the two geometries from the
`swaths` layer. The complete paired scene is retained when either side reaches
the requested threshold:

```text
scene_selected = (clear_sky_left >= threshold)
              OR (clear_sky_right >= threshold)
```

The other swath is never removed from a selected scene, even when it is cloudy.
This allows the catalogue to preserve the widest possible SWOT/OLCI vignette
while clearly identifying which side passed the filter.

```powershell
.\.venv\Scripts\python.exe scripts\screen_olci_clear_sky.py `
  --catalog "outputs\s3a_swot_2026.gpkg" `
  --catalog "outputs\s3b_swot_2026.gpkg" `
  --credentials "C:\path\outside\the\repository\credentials.txt" `
  --temporary-directory "data\temporary_olci_screening" `
  --clear-sky-min 90 `
  --min-width-km 25 `
  --min-length-km 25 `
  --output "outputs\olci_swot_2026_clear_sky.gpkg"
```

The screening output reports `clear_sky_percent_left` and
`clear_sky_percent_right`, their pixel counts, two Boolean pass fields,
`clear_sky_percent_best`, and `selected_swaths`. Its `intersections` layer
always contains the full paired scene; its `swaths` layer retains both
component geometries. Temporary browse, tie-point, and WQSF files are deleted
product by product unless `--keep-temporary` is supplied.
...
Use the native paired-swath GeoPackage produced in step 5; the section order
keeps the conversion physics before the extraction details, but the native
refinement must be executed first.
...
## 5. Refine and extract the paired SWOT scene
...
The script uses the ORF scene only as a seed, relocates both sides with native
KaRIn geolocation, and writes
`swot_subset_left_native.nc` and `swot_subset_right_native.nc`. Each file
contains native SWOT pixels, geolocation, time, `ssh_karin_2`, the separate
crossover calibration and its quality flag, SSHA, Sigma0, quality flags,
surface classification, cross-track distance, and an exact `in_vignette` mask.
The refined GeoPackage keeps the two side geometries under the same
`vignette_id`.
...
for the pixel-weighted union. With the paired-swath selection rule, the
complete scene is retained because the right swath passes the 90% threshold;
the cloudier left swath remains part of the same scene and is not discarded.
...
The one scene identifier selects both disjoint native swath geometries. When
plotting, repeat `--subset` so each two-dimensional KaRIn grid retains its
...
The TCWV product is generated for cloud-free daytime pixels. Clear-sky coverage
is calculated separately for the left and right SWOT swaths:

```text
clear_sky_side =
    valid TCWV pixels inside that side / OLCI pixels inside that side
```

and separately over ocean when required. A scene passes when at least one side
meets `--clear-sky-min`; both side geometries and both percentages remain in
the selected output. The converted output preserves an exact `in_vignette`
mask and masks the relevant TCWV/IWV quality flags, so these percentages can be
computed without resampling the sensor grid. A product browse image remains
useful only for rapid qualitative screening.
...
The tests cover ORF parsing and interpolation, paired left/right scene
geometry, the clear-sky OR selection rule, tangent geometry handling,
